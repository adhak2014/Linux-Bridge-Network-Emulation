...
echo ""
echo "Assign 10.200.1.2/24 to vpeer1"
ip netns exec h1 ip addr add 10.200.1.2/24 dev vpeer1
echo ""
...
iptables -t nat -F
echo ""
echo "Enable masquerading of 10.200.1.0"
iptables -t nat -A POSTROUTING -s 10.200.1.2/24 -o enp0s3 -j MASQUERADE
...
echo "\n"
echo "From h1, check ip route to 8.8.8.8"
ip netns exec h1 traceroute 8.8.8.8
echo "\n"
echo "From h1, test IP connectivity to 8.8.4.4"
ip netns exec h1 ping -c4 8.8.4.4
echo "\n"
echo "From h1, check ip route to 8.8.4.4"
ip netns exec h1 traceroute 8.8.4.4
echo "\n"
